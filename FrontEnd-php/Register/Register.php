...
<!--flecha volver-->
<a href="../Login/Login.php" id="a-back"><</a>
<div id="div-all">
...
$name = $password = $passwordConf = $email = $emailConf = $day = $month = $year ="";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" /*&& !$_SESSION["newsession"]*/) {
    ...
    if (empty($_POST["email"])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if (empty($_POST["emailConf"])) {
                $emailConfErr = "Confirm your email is required";
            } else {
                $emailConf = test_input($_POST["emailConf"]);

                if ($email != $emailConf) {
                    $emailConfErr = "Error whit confirm your email";
                }
            }
        }
    }
        ...
        if(strlen($password) < 8){
            ...
            if (!preg_match("/^[a-zA-Z0-9' ]*$/",$password)) {
                ...
                if (empty($_POST["passwordConf"])) {
                    $passwordConfErr = "Confirm your password is required";
                } else {
                    $passwordConf = test_input($_POST["passwordConf"]);

                    if ($password != $passwordConf) {
                        $passwordConfErr = "Error whit confirm your password";
                    }
                }
            }
        }
    ...
    if (empty($_POST["day"]) || empty($_POST["month"]) || empty($_POST["year"])){
        $calenderErr = "Complete your born date";
    } else {
        if(!checkdate($month ,$day ,$year)){
            $calenderErr = "Set a correct date";
        }
    }
