...
def resizeFullScreenImage(urlImage):
    ...
    if(rootWidth / width < rootHeight / height):
        image_resized = image_no_resized.resize(
            (int(rootWidth*0.8), int((height/width*rootWidth)*0.8)), Image.ANTIALIAS)
    else:
        image_resized = image_no_resized.resize(
            (int((width/height*rootHeight)*0.8), int(rootHeight*0.8)), Image.ANTIALIAS)
...
labelimagemain.place(relx=0.5, rely=0.5, relwidth=1,
...
if rotateEnable == 'False':
    ...
    # Hide Show Buttons
    hide_show_button = Button(root, text="Hide Buttons", font=(
        "Verdana", 10, "bold"), command=change_display_buttons, border=0, bg="#00a797", fg="#242424")
...
if infoEnable == 'True':
    info_container = Frame(root, bg="#3d3d3d", width=int(
        rootWidth), height=int(rootHeight/10))
    info_container.place(relx=0.5, rely=0.05, anchor=CENTER)

    artistLabel = Label(root, text="ArtistName", font=(
        "Verdana", 12, "bold"), bg="#3d3d3d", fg="white")
    artistLabel.place(relx=0.5, rely=0.08, anchor=CENTER)

    pictureNameLabel = Label(root, text="PictureName", font=(
        "Verdana", 30, "bold"), bg="#3d3d3d", fg="white")
    pictureNameLabel.place(relx=0.5, rely=0.035, anchor=CENTER)

    # Hide Show Info
    hide_show_info = Button(root, text="Hide Info", font=(
        "Verdana", 10, "bold"), command=change_display_info, border=0, bg="#00a797", fg="#242424")
...
button_close = Button(root, command=openSettings,
                      image=close_image, highlightthickness=0, bd=0)
